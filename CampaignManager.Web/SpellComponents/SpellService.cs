    ...
    ILogger<SpellService> logger)
{
    ...
    {
        if (cache.TryGetValue(SpellsKey, out List<Spell> cachedSpells))
        {
            return cachedSpells;
        }
    ...
    public async Task<bool> AddSpellAsync(Spell spell)
    {
        try
        {
            ...
            if (await dbContext.Spells.AnyAsync(s => s.Name.Equals(spell.Name, StringComparison.OrdinalIgnoreCase)))
            {
                return false;
            }
