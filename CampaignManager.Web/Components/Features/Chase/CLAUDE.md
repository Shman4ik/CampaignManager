...
`Resolve*` / `MoveForward` — **чистые функции**: считают бросок и заполняют
...
Самая частая форма результата — «бросок против навыка, исполнитель остаётся на своей локации»
(атаки, манёвр, проверка управления, создание помехи). Её собирает `SkillCheckResult`
в `ChaseService.Rules.cs`; поля, специфичные для действия (`Summary`, урон, потерянные действия),
дописываются после вызова. Не выписывать инициализатор `ChaseActionResult` заново, если действие
укладывается в эту форму.
...
- **Своя СКО для способа передвижения важнее половины обычной** (стр. 141). Книга печатает
  у тварей «6 / полёт 20», и эти числа лежат в `CreatureCharacteristics.SwimSpeed` /
  `FlySpeed`, откуда попадают в `ChaseParticipant.SwimSpeed` / `FlySpeed`. `BaseMov` берёт
  сначала введённую Хранителем `NativeModeSpeed`, потом статблок, и только потом делит
  обычную СКО пополам. Без этого бьякхи летел со скоростью 2 вместо 16.
