    ...
    ILogger<ScenarioNpcService> logger)
{
    ...
    public async Task<ScenarioNpc?> CreateNpcAsync(ScenarioNpc npc)
    {
        try
        {
            ...
            var scenarioExists = await dbContext.Scenarios.AnyAsync(s => s.Id == npc.ScenarioId);
            if (!scenarioExists)
            {
    ...
    public async Task<bool> UpdateNpcAsync(ScenarioNpc npc)
    {
        try
        {
            ...
            var existingNpc = await dbContext.ScenarioNpcs.FindAsync(npc.Id);
            if (existingNpc is null) return false;
    ...
    public async Task<bool> DeleteNpcAsync(Guid id)
    {
        try
        {
            ...
            var npc = await dbContext.ScenarioNpcs.FindAsync(id);
    ...
    public async Task<bool> LinkNpcToCharacterAsync(Guid npcId, Guid characterId)
    {
        try
        {
            ...
            var npc = await dbContext.ScenarioNpcs.FindAsync(npcId);
            if (npc is null) return false;

            // Verify that the character exists
            var characterExists = await dbContext.CharacterStorage.AnyAsync(c => c.Id == characterId);
            if (!characterExists)
            {
