...
- **Музыка локации.** `ScenarioLocation.MusicTags` и `MusicTrackIds` — теги настроения и
  прибитые гвоздями треки; в режиме игры по ним `LocationMusicPanel` набирает пул и берёт
  случайный трек. Оба поля едут внутри jsonb-колонки `Scenarios.Locations`, миграции не требуют.
  Подробности о самой фонотеке — в `Music/CLAUDE.md`.
- **Новое поле локации обязано попасть в глубокое копирование в `LocationEditorModal.Show()`.**
  Там уже копируются `CreatureIds`/`ItemIds`/`NpcIds`/`HandoutIds`/`MusicTags`/`MusicTrackIds`;
  забытое поле не падает, а молча стирается при любой правке локации.
- `AddLocationAsync`/`UpdateLocationAsync`/`RemoveLocationAsync` **сбрасывают кэши сценариев**
  (`InvalidateScenarioCaches`). Раньше не сбрасывали, и правка локации не доезжала до режима игры
  до истечения пятнадцатиминутного кэша — заметили на тегах музыки, но касалось это всех правок.
