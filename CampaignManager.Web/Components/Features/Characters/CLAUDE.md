...
- `CharacterService(dbContextFactory, identityService, logger, IMemoryCache)` — CRUD.
  ...
  внутри jsonb, а он не колонка и в SQL не разбирается.
...
- `LuckRules` (static) — необязательное правило «Пункты Удачи» (стр. 96): во что обойдётся
  дотянуть бросок до обычного/трудного/чрезвычайного успеха. Уровень успеха не считает сам, а
...
- `OccupationService(dbContextFactory, IMemoryCache, logger)` — occupation catalog (skill point formulas, tags).
  ...
  **Сопоставление идёт по имени и только по имени**: переименуешь профессию в коде — синхронизация
  заведёт новую строку, а старая останется рядом. Так уже было с «Детектив» → «Детектив полиции»:
  на время переноса в сервисе жила табличка синонимов, сейчас её нет и заводить снова не нужно —
  такие случаи разовые, их проще довести руками на `/occupations`.
...
- `FellowInvestigator` — строка блока «Знакомые сыщики» (`Character.FellowInvestigators`).
  У строки из состава кампании заполнен `CharacterId`, и своей в ней остаётся только `Note`:
  имя, игрок и профессия каждый раз подставляются из живого листа соседа. `Name`/`PlayerName`
  в такой строке — слепок на случай, если сосед выйдет из кампании, иначе заметка осталась бы
  без адресата. У вписанного руками `CharacterId` пуст, и тогда вся строка — данные этого листа.
- `PersonalInfo.PortraitUrl` — портрет сыщика: ключ объекта в MinIO (`images/characters/…`) либо
  внешний https-адрес; показывается через `/api/minio/image/{путь}`, как изображения бестиария.
  Удаление портрета с листа стирает ссылку, но не объект: лист мог быть скопирован (преген →
  лист игрока), и удаление из хранилища осиротило бы чужой портрет.
...
- **Портрет** (`CharacterPortrait`) стоит слева от полей «Имя/Профессия» — как рамка в правом
  верхнем углу бланка. Рамка сама по себе `<label>` для скрытого `InputFile` (большая тап-цель),
  но подпись «Загрузить»/«Заменить» под ней обязательна — наведения на планшете нет, и без
  подписи рамка читается как украшение.
- **Подписи полей анкеты стоят над полями, а не слева.** Слева подпись съедала 70px из ширины
  колонки, и из-за этого приходилось прятать портрет до `lg` и складывать пары полей (Возраст/Пол,
  Б.К.У./Комп., Скор./Укло.) в столбик: имя ужималось до 46px, «Джон Смит» превращался в «Дж».
  Возвращать подпись влево — значит вернуть и обе заплатки.
- **Три колонки карточки начинаются с `lg` (1024px), а не с `md`.** На 768px они помещались только
  пока кнопки шага ПЗ были 20x24: с нормальной тап-целью «Состоянию» остаётся 240px, и колонка
  «Макс.» обрезает «11» до «1». Ниже `lg` блоки идут в столбик, а колонки характеристик и
  состояния упираются в `max-w-md` — растянутое на 688px поле под двузначное число читается как
  ошибка вёрстки. Проверять правки этой карточки надо на 768px, 1024x1366 и 1366x1024.
- **«Труд.» и «Чрез.» в характеристиках — текст, а не поля ввода** (`.attr-derived`). Их считает
  `AttributeValue` из «Обыч.», и шестнадцать отключённых рамок занимали половину таблицы, при этом
  выглядя редактируемыми.
- **Б.К.У., Комплекция, Скорость и Уклонение живут под характеристиками**, в блоке «Производные»,
  а не в анкете: их считает `DerivedAttributeRules` из характеристик, и место под таблицей всё
  равно пустовало. «Укло.» там только зеркалит навык — правится в разделе «Навыки».
- **Тап-цели в «Состоянии»:** шаг ПЗ/ПМ/Рассудка/Удачи — штатный `cm-btn cm-btn-sm cm-btn-icon`
  (36x36) с обязательным `shrink-0`: без него кнопка сжимается как flex-элемент в узкой колонке
  и снова становится 21px. Отметки состояния — строки в 40px, чекбокс внутри штатный `cm-checkbox`.
- **Колонки отметок состояния — это группы, а не половинки списка.** Слева `_bodyConditions`
  (серьёзная рана → без сознания → при смерти: порядок причинный, рана решает, чем обернётся
  ноль ПЗ), справа `_mindConditions` (временное безумие → бессрочное). Новая отметка кладётся
  в свой список и попадает в свою колонку, не сдвигая соседнюю.
- **Встают колонки рядом по `@container`, а не по медиазапросу** (`CharacterStateTable.razor.css`).
  Ширина блока меняется не так, как ширина окна: ниже `lg` он идёт во всю ширину листа (448px,
  две колонки), а на 1024px это самая узкая из трёх колонок (307px, одна). Медиазапрос дал бы
  две колонки ровно там, где они не помещаются: самой длинной подписи «Бессроч. безумие»
  с чекбоксом и значком нужно около 190px. В одну колонку группы просто встают друг под друга
  с тем же шагом и читаются как один список.
- **Уведомление листа — всплывающая плашка `.sheet-toast`**, а не блок в потоке: она висит под
  шапкой у кнопки сохранения, рядом со значком автосохранения, и ничего не двигает. `info` и
  `success` уходят сами через `NotificationLifetime` (5 с), `warning` и `error` ждут Хранителя —
  в них текст, по которому надо что-то сделать. Таймер живёт в `ShowNotification`, снимается в
  `ClearNotification` и в `DisposeAsync`.
- **«Знакомые сыщики»** (`FellowInvestigatorsPanel`) — в секции «Биография и заметки», поэтому у
  секции `BodyClass="p-4 cm-stack"`: линейка отделяет блок от биографии вместо второй карточки.
  Строки списка — состав из `CharacterService.GetPartyAsync` плюс вписанные руками; удалить можно
  только вписанные. **Заметка о соседе заводит строку в JSONB лишь в момент, когда её напишут**:
  автосохранение сверяет слепок листа, и заранее созданные пустые строки означали бы, что простое
  открытие листа уходит в базу.
- **Патроны и Неиспр. — колонки таблицы оружия**, а не поля раскрытых деталей: на бланке это
  колонки, и в бою в них смотрят каждый раунд. Таблица из-за этого едет вбок внутри своего
  `overflow-x-auto`, а у строки деталей `colspan="9"` — добавишь колонку, поправь и его.
  Показывается она с `lg`, а не с `md`: девяти колонкам на 768px тесно (на название оружия
  оставалось 71px), и до 1024px вместо таблицы идут карточки с теми же графами.
...
- **Имена навыков в `Occupation.OccupationSkills` пишутся ровно как в справочнике навыков**
  («Язык, иностранный», не «Языки (иностр.)»). Словаря синонимов больше нет: он жил ровно столько,
  сколько в базе оставались старые написания, и удалён после синхронизации. Навык, который не
  нашёлся, превращается в слот `Unresolved` — игрок выбирает замену руками, а Хранитель видит
  расхождение на `/occupations`. Это и есть защита от опечатки; чинить её новым словарём синонимов
  не надо, надо поправить имя в профессии.
