...
- **Портрет** (`CharacterPortrait`) стоит справа от полей «Игрок/Имя/Профессия» — как рамка в
  правом верхнем углу бланка, — и **показывается только с `lg`** (1024px, iPad Pro в портрете).
  Ниже он не влезает, и его там просто нет: прячется рамка, а не `PortraitUrl`, так что
  загруженный портрет никуда не девается. Рамка сама по себе `<label>` для скрытого `InputFile`
  (большая тап-цель), но подпись «Загрузить»/«Заменить» под ней обязательна — наведения на
  планшете нет, и без подписи рамка читается как украшение.
- **Ширина колонки «Персональная информация» — не ширина окна.** Сетка карточки разъезжается на
  три колонки с `md` (768px), и на этой ширине колонка сама всего 216px. Рамка портрета рядом с
  полем зажимала там имя до 46px («Джон Смит» → «Дж»), а парные строки (Возраст/Пол, Б.К.У./Комп.,
  Скор./Укло.) давали по 50px на поле — поэтому портрет живёт с `lg`, а пары до `lg` складываются
  в столбик. Ронять рамку под поля по `flex-wrap` пробовали: ширину это лечит, но рамка встаёт
  между «Профессией» и «Возрастом» и читается как случайный блок посреди формы. Проверять правки
  этой карточки надо и на 768px, не только на двух ориентациях iPad Pro.
