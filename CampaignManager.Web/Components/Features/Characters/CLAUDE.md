...
- **Портрет** (`CharacterPortrait`) стоит внутри `UnifiedPersonalInfoCard` справа от полей
  «Игрок/Имя/Профессия» — там же, где рамка на бланке. Своей колонки он не получает: в портретной
  ориентации iPad их и так три. Рамка сама по себе `<label>` для скрытого `InputFile` (большая
  тап-цель), но подпись «Загрузить»/«Заменить» под ней обязательна — наведения на планшете нет,
  и без подписи рамка читается как украшение.
