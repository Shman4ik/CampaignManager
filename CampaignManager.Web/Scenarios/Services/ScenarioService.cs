    ...
    ILogger<ScenarioService> logger)
{
    ...
    public async Task<List<Scenario>> GetAllScenariosAsync(bool templatesOnly = false)
    {
        try
        {
            string cacheKey = templatesOnly ? TemplatesCacheKey : ScenariosCacheKey;

            if (cache.TryGetValue(cacheKey, out List<Scenario>? scenarios) && scenarios is not null)
            {
                return scenarios;
            }

            using AppDbContext dbContext = await dbContextFactory.CreateDbContextAsync();
            IQueryable<Scenario> query = dbContext.Scenarios.AsQueryable();
    ...
    public async Task<bool> UpdateScenarioAsync(Scenario scenario)
    {
        try
        {
            ...
            Scenario? existingScenario = await dbContext.Scenarios.FindAsync(scenario.Id);
    ...
    public async Task<bool> DeleteScenarioAsync(Guid id)
    {
        try
        {
            ...
            Scenario? scenario = await dbContext.Scenarios.FindAsync(id);
            if (scenario is null)
            {
                return false;
            }
    ...
    public async Task<Scenario?> CreateScenarioFromTemplateAsync(Guid templateId, Guid campaignId, string creatorEmail)
    {
        try
        {
            ...
            Scenario? template = await dbContext.Scenarios
                .Include(s => s.Npcs)
                .Include(s => s.ScenarioCreatures)
                .Include(s => s.ScenarioItems)
            ...
            // Clone NPCs
            foreach (ScenarioNpc npc in template.Npcs)
            {
            ...
            foreach (ScenarioCreature sc in template.ScenarioCreatures)
            {
            ...
            foreach (ScenarioItem si in template.ScenarioItems)
            {
