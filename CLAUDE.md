...
Поверх неё лежит `20260910141633_DropLlmKnowledgeEntries` — удаление таблицы вместе с
выпиленной LLM-валидацией персонажа. **Она уничтожает данные:** `Down()` восстанавливает
только структуру. В таблице около 31 КБ авторского справочника CoC 7e на русском, к самой
валидации отношения не имеющего, — выгрузить его до `database update` помогает
